#!/usr/bin/env python3
"""
dishost.py - Enhanced IP Range Scanner with Configurable Health Checks
This script scans IP ranges and performs health checks using various protocols.
Results can be displayed on screen and saved in JSON, CSV, or XML formats.
"""
import argparse
import csv
import ipaddress
...
class UDPHealthChecker(HealthChecker):
    """UDP port health checker."""
    
    def __init__(self, port: int, timeout: float = 1.0, data: bytes = b"test"):
        """
        Initialize UDP health checker.
        
        Args:
            port: UDP port to check
            timeout: Connection timeout in seconds
            data: Data to send in the UDP packet
        """
        super().__init__()
        self.port = port
        self.timeout = timeout
    ...
    def check(self, ip: str) -> Dict:
        ...
        try:
            start_time = time.time()
            sock.sendto(self.data, (ip, self.port))
            try:
                ...
                # No response received, but port might be open
...
class HTTPHealthChecker(HealthChecker):
    ...
    def __init__(
        ...
        timeout: float = 3.0,
        user_agent: str = "dishost-scanner/1.0"
    ):
        """
...
class IPScanner:
    ...
    def __init__(
        ...
        Args:
            ...
            scan_delay: Delay between each IP scan in seconds
    ...
    def generate_ips_from_file(self, filename: str) -> List[str]:
        """
        Generate IP addresses from a file.
        
        Args:
            filename: Path to file containing IPs or CIDRs
        ...
        try:
            with open(filename, 'r') as f:
                for line in f:
                    ...
                    if '/' in line:
                        ips.extend(self.generate_ips_from_cidr(line))
                    else:
                        # Validate IP address
                        ipaddress.IPv4Address(line)
                        ips.append(line)
        except (IOError, ValueError) as e:
            raise ValueError(f"Error reading IP file: {e}")
        
        return ips
    
    def exclude_ips(self, ips: List[str], exclude_file: str) -> List[str]:
        """
        Exclude IPs from the list based on exclude file.
        
        Args:
            ips: List of IP addresses to filter
            exclude_file: Path to file containing IPs or CIDRs to exclude
        ...
        try:
            with open(exclude_file, 'r') as f:
                for line in f:
                    ...
                    # Check if it's a CIDR
                    if '/' in line:
                        exclude_networks.append(ipaddress.ip_network(line, strict=False))
                    else:
                        # Convert single IP to /32 network
                        exclude_networks.append(ipaddress.ip_network(f"{line}/32", strict=False))
        except (IOError, ValueError) as e:
            raise ValueError(f"Error reading exclude file: {e}")
        
        # Filter out IPs that are in any exclude network
        filtered_ips = []
        for ip in ips:
    ...
    def _check_with_retry(self, ip: str) -> Dict:
        """Perform health check with retries."""
        result = None
        
        for attempt in range(self.retries + 1):
            try:
                result = self.health_checker.check(ip)
                if result["status"] == "up" or attempt == self.retries:
                    break
                
                if self.verbose:
                    self.console.print(f"Retrying {ip} (attempt {attempt + 1})...")
                ...
                if attempt == self.retries:
                    result = {
                        "ip": ip,
                        "protocol": getattr(self.health_checker, 'protocol', 'unknown'),
                        "status": "error",
                        "error": str(e)
                    }
                else:
                    if self.verbose:
                        self.console.print(f"Error checking {ip}: {e}, retrying...")
                    time.sleep(self.retry_delay)
        
        # Add delay between IP scans if specified
        ...
        if self.show_progress:
            with Progress(
                ...
                with ThreadPoolExecutor(max_workers=self.max_threads) as executor:
                    for result in executor.map(self._check_with_retry, ips):
                        results.append(result)
                        progress.update(task, advance=1)
        else:
            with ThreadPoolExecutor(max_workers=self.max_threads) as executor:
                results = list(executor.map(self._check_with_retry, ips))
...
class ResultManager:
    ...
    def display_results(self, results: List[Dict]):
        ...
        for result in results:
            ...
            if protocol == "icmp":
                ...
                port = result.get("port", "N/A")
                latency = result.get("latency", "N/A")
                response_size = result.get("response_size", "N/A")
                details = f"port: {port}, latency: {latency}ms, response: {response_size} bytes"
    ...
    def save_as_xml(self, results: List[Dict], filename: str):
        """
        Save results as XML.
        ...
        ET.SubElement(metadata, "total_hosts").text = str(len(results))
        ET.SubElement(metadata, "up_hosts").text = str(sum(1 for r in results if r["status"] == "up"))
        ET.SubElement(metadata, "down_hosts").text = str(sum(1 for r in results if r["status"] == "down"))
        ...
        for result in results:
            ...
            for key, value in result.items():
                ...
                field = ET.SubElement(result_elem, key)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="dishost - Enhanced IP Range Scanner with Health Checks",
...
Examples:
  ...
  # Scan with UDP port check
  dishost.py --cidr 192.168.1.0/24 --check udp --port 53
  
  # Scan with HTTP check
  dishost.py --cidr 192.168.1.0/24 --check http --port 80 --path /health --expect-code 200
  
  # Load IPs from file and exclude certain IPs
  dishost.py --file targets.txt --exclude exclude.txt --check icmp
  
  # Save results in multiple formats
  dishost.py --cidr 192.168.1.0/24 --check icmp --json results.json --csv results.csv --xml results.xml
"""
    )
    
    # IP range arguments
    ip_group = parser.add_argument_group('IP Range (required, use either CIDR, start-end, or file)')
    ...
    ip_ex_group.add_argument('--file', help='File containing IPs or CIDRs (one per line)')
    ...
    check_group.add_argument('--port', type=int, help='Port for TCP/UDP/HTTP check')
    ...
    http_group.add_argument('--user-agent', default='dishost-scanner/1.0', help='User-Agent string for HTTP requests')
    ...
    output_group.add_argument('--xml', help='Save results to XML file')
    ...
    control_group = parser.add_argument_group('Scan Control Options')
    control_group.add_argument('--retries', type=int, default=0, help='Number of retries for failed checks (default: 0)')
    control_group.add_argument('--retry-delay', type=float, default=0.5, help='Delay between retries in seconds (default: 0.5)')
    control_group.add_argument('--scan-delay', type=float, default=0.0, help='Delay between each IP scan in seconds (default: 0.0)')
    control_group.add_argument('--exclude', help='File containing IPs or CIDRs to exclude (one per line)')
    ...
    if args.check in ('tcp', 'udp', 'http', 'https') and not args.port:
    ...
    try:
        ...
        if args.cidr:
            ...
            ip_list = IPScanner(None).generate_ips_from_file(args.file)
        
        # Exclude IPs if specified
        if args.exclude:
            ip_list = IPScanner(None).exclude_ips(ip_list, args.exclude)
        ...
        if args.check == 'icmp':
            ...
            health_checker = UDPHealthChecker(
                port=args.port,
                timeout=args.timeout
            )
        ...
        if args.xml:
            result_manager.save_as_xml(results, args.xml)
